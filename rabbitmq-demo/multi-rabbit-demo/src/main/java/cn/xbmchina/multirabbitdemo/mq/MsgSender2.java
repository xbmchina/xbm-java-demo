...
@Component
@Slf4j
public class MsgSender2 implements ConfirmCallback {

	//由于rabbitTemplate的scope属性设置为ConfigurableBeanFactory.SCOPE_PROTOTYPE，所以不能自动注入?
	//如果需要使用secondMQTemplate，这里就配置secondMQTemplate
    private RabbitTemplate secondMQTemplate;

	public MsgSender2(@Qualifier(value = "secondMQTemplate")RabbitTemplate firstMQTemplate) {
		this.secondMQTemplate=firstMQTemplate;
	...
	public void sendTopicMsg(String topicExchange, String routingKey, Object content) {
		...
		try {
			secondMQTemplate.convertAndSend(topicExchange, routingKey, content, correlationId);
		}catch(Exception e) {
